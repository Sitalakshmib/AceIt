...
def generate_ai_resume_feedback(resume_text: str, job_role: str, rule_based_analysis: dict) -> dict:
    """
    Use Gemini to generate intelligent, personalized resume feedback
    """
    if not llm_client:
        return {
            "ai_feedback": None,
            "error": "AI analysis unavailable - LLM client not initialized"
        }
    
    # Create a comprehensive prompt for Gemini
    prompt = f"""You are an expert resume reviewer and career coach. Analyze the following resume for a {job_role.replace('_', ' ')} position and provide detailed, actionable feedback.

RESUME TEXT:
{resume_text[:4000]}  # Limit to avoid token limits

CURRENT ANALYSIS SUMMARY:
- Overall Score: {rule_based_analysis.get('overall_score', 'N/A')}/100
- ATS Score: {rule_based_analysis.get('ats_analysis', {}).get('ats_score', 'N/A')}/100
- Skills Match: {rule_based_analysis.get('skills_analysis', {}).get('match_score', 'N/A')}%
- Matched Skills: {', '.join(rule_based_analysis.get('skills_analysis', {}).get('matched_skills', [])[:5])}
- Missing Skills: {', '.join(rule_based_analysis.get('skills_analysis', {}).get('missing_skills', [])[:5])}

Please provide your analysis in the following JSON format:
{{
    "overall_impression": "A 2-3 sentence summary of the resume's strengths and weaknesses",
    "strengths": ["List 3-5 specific strengths of this resume"],
    "areas_for_improvement": ["List 3-5 specific areas that need improvement"],
    "actionable_tips": ["List 5 specific, actionable tips to improve this resume for the {job_role.replace('_', ' ')} role"],
    "keyword_suggestions": ["List 5-8 keywords/skills the candidate should add for better ATS compatibility"],
    "formatting_advice": "Specific advice about resume formatting and structure",
    "interview_readiness": "Assessment of how well this resume would lead to interview callbacks (Low/Medium/High) with brief explanation"
}}

Be specific, constructive, and reference actual content from the resume where possible. Focus on what would make this candidate more competitive for a {job_role.replace('_', ' ')} position."""
    ...
    try:
        ...
        print(f"[ERROR] AI Resume Analysis Error: {e}")
        return {
            "ai_feedback": None,
            "error": str(e)
        }
...
async def analyze_resume(
    ...
    # Compile rule-based analysis results first
    analysis = {
        "job_role": job_role,
    ...
    ai_result = generate_ai_resume_feedback(resume_text, job_role, analysis)
