...
from database import find_user_by_email, add_user
...
async def register(user: User):
    ...
    user_dict["created_at"] = datetime.utcnow()  # Add timestamp

    # Save user to in-memory database
    add_user(user_dict)
    print(f"User {user.email} registered successfully with ID: {user_dict['id']}")
    return {"message": "User created successfully", "user_id": user_dict["id"]}
